...
- 环境：Ubuntu 22.04，Python 3.12.3，PyTorch 2.8.0+cu128，CUDA runtime 12.8
- 训练框架：LLaMA-Factory 0.9.6.dev0；源码来自官方 GitHub codeload 归档，SHA-256 另见 llamafactory-source.sha256
- 数据盘：120GB；收尾时约使用 15GB
...
- 确定性边界：LoRA 只负责生成查询计划；可靠性仍由 JSON Schema 校验、工具白名单、只读执行和安全回退共同保障，不依赖 LLM 自觉

### 3. TRPG 领域 Embedding 微调
...
- 同机平均单查询编码延迟：base 约 8.902ms，tuned 约 9.399ms；延迟不跨硬件比较
...
- 产物哈希：/root/autodl-tmp/training-run/artifact-manifest.sha256
