...
所以先在 `demo/` 里把「权威内核 + 跑团桌」这条纵向切片做干净，让它可玩、可测、可重放。桌面外壳已定为 Electron（`electron/`）。`demo/` 不引入 Electron 也能在浏览器里跑。
...
选型定了：浏览器里用官方的 `@sqlite.org/sqlite-wasm` 走 kvvfs。Electron 主进程用 `better-sqlite3` 写真正的 SQLite 文件（`electron/sql/` 是 V1 DDL）。两套驱动，仓储形状对齐，但战役表已经按 V1 在主进程另写一份，不是简单换文件。
...
- 后台任务调试面板：主持人选项里勾「调试后台任务」，记录栏画出 Information / Director / Memory 冷路径（程序 / 模型 / 跳过）。默认关。
...
2. **Demo 与桌面的归并。** 已定 Electron。权威在主进程，渲染只走 `desktopApi`。`electron/` 已有战役目录和 V1 DDL；下一步把 `session.act` 改成 `turn:submitAction`。浏览器里开 Demo 仍走 wasm 库。
