...
import type { DesktopApi, DesktopCampaign } from "@renderer/desktop";
import { createFreshDesktopCampaign, loadDesktopBranch, loadDesktopCampaign, submitDesktopTurn } from "@renderer/desktop-play";
...
test("desktop candidate event reaches the sheet before the resolved turn view", async () => {
  ...
  let listener: ((event: import("@renderer/desktop").OperationEvent) => void) | undefined;
  const candidate = {
    title: "寻找让列车员开口的破绽",
    skill: "侦查",
    skillValue: 65,
