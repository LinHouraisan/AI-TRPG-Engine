...
import { createFreshDesktopCampaign, ensureDesktopCampaign, loadDesktopBranch, loadDesktopCampaign, loadDesktopEvents, submitDesktopTurn, tryDesktopApi } from "@renderer/desktop-play";
import { emptyMemory, runAfterCommit, runAfterCommitLive, traceFromJobs, type JobTrace, type MemoryState } from "@core/ai";
...
import { emptyContextStore, type ContextStore } from "@core/engine/context-store";
import { narrate, openingLine, suggest } from "@core/engine/narrate";
...
import type { CheckCandidate, CheckResult, EventDraft, GameEvent, GameState, Intent } from "@core/engine/types";
import { loadConfig, saveConfig, type KeeperConfig } from "@core/keeper/config";
import type { ContextUsage } from "@core/keeper/context";
...
import { confirmationReducer, initialConfirmationState } from "@renderer/ui/investigator-creation-state";
import type { DesktopCampaignBackup } from "@renderer/desktop";
  ...
  const act = useCallback(
    async (intent: Intent, spoken: string, held = false, modelTaskId?: string) => {
      ...
      if (remote && campaignId && branchId) {
        ...
        const check = view.check as import("@core/engine/types").CheckResult | undefined;
