...
const ACTIVE_PACK_KEY = "ai-trpg-engine/active-pack";
...
function scanPacksWithBun(): Record<string, unknown> {
  const packsDir = new URL(/* @vite-ignore */ "../../../content/packs/", import.meta.url);
...
const globbedJson: Record<string, unknown> =
  typeof Bun !== "undefined"
    ...
    : isElectronProcess()
      ? scanPacksWithNode()
      : import.meta.glob("../../../content/packs/*/*.json", { eager: true });
  ...
  try {
    ...
    throw new Error(
      `模组「${id}」字段对不上规范，不能开团：${formatSchemaError(error)}。` +
        "对照 content/packs/SPEC.md 改这份 JSON，再跑 bun run pack:lint。",
    );
  }
...
function activePackSource(): PackSource {
  ...
  if (!found) {
    throw new Error(
      `找不到当前生效的模组「${id}」。` +
        `确认 content/packs/${id}/ 下齐了八个 JSON，` +
        "缺一份引擎就没法开团。",
    );
  }
  return found.source;
