...
**现状（原型，未接内容库）：** `electron/src/core/cards/` 是 CharacterCardImporter 的窄切片。读 SillyTavern 卡，输出 `CardImportDraft`（`capability: character_card`，`confirmed: false`）。自动车把属性／技能标成 `generated`。世界书只用来加关键词技能，不单独走 WorldBookImporter。没有 staging、没有 `installed_content`、没有用户确认后的安装。详见 `electron/src/core/cards/README.md`。
