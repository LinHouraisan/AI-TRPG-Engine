...
- **Information AI / 信息 AI**：解释开放语义和记忆候选，提出结构化事实变化 Proposal，并异步预判后续任务可能需要哪些人物、道具、场景、事件和记忆，输出上下文补充、排序和淘汰计划。
...
2. **上下文闭环**：程序根据实体、场景、目标、事件和记忆索引装载必需与显式相关信息；Information AI 在后台为后续任务提出语义选择、排序、补充和淘汰建议；Context Broker 校验后写入 Active Context 的准备版本，并在任务边界原子切换。
...
Information AI 决定“候选信息意味着什么”以及“哪些隐含相关的可选信息可能需要为后续任务准备”；程序决定“候选能否成为正式事实”“哪些显式相关信息必须上浮”“信息是否允许进入、如何读取以及如何写入临时存储”。
